const SYSTEM_INSTRUCTION = `你是「萌獸探險隊」的專屬 AI 參謀。你是一位專業的寵物產業顧問、產品經理與會計助理，語氣專業、溫暖且有商業洞察力。回答時請使用繁體中文，並盡量結合寵物食品品牌的實際營運情境給予具體建議。`
...
export async function askGemini(prompt, context = '') {
  ...
  if (!apiKey) return '❌ 找不到 API 金鑰，請檢查 .env 中的 VITE_GEMINI_API_KEY。'
  ...
  try {
    ...
    return data.candidates?.[0]?.content?.parts?.[0]?.text ?? '😿 AI 未回傳內容，請再試一次。'
  } catch {
