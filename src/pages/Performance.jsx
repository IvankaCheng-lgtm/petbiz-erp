...
export default function Performance({ data }) {
  ...
  const marketItemPerEvent = useMemo(() => {
    const mktRevs = filteredRevenues.filter(r => r.channel === '市集' && r.items)
    const allMap = {}
    mktRevs.forEach(r => {
      ;(r.items || []).forEach(it => {
  ...
  const allItemStats = useMemo(() => {
    ...
    // 市集現場收款
    filteredRevenues.filter(r => r.channel === '市集' && r.items).forEach(r => {
      ;(r.items || []).forEach(it => {
        if (!map[it.itemId]) map[it.itemId] = { name: it.itemName, total: 0, amount: 0, ec: 0, market: 0, offline: 0, byPlatform: {} }
        map[it.itemId].total += it.qty
        map[it.itemId].amount += it.qty * it.unitPrice
  ...
  function handlePerformancePrint() {
    ...
    const channelRows = channelData.map(ch =>
      ...
      '<td style="padding:8px 12px;text-align:right">' + fmt(ch['營收']) + '</td>' +
      '<td style="padding:8px 12px;text-align:right;color:' + (ch['毛利'] >= 0 ? '#059669' : '#dc2626') + ';font-weight:700">' + fmt(ch['毛利']) + '</td>' +
      '<td style="padding:8px 12px;text-align:right;color:' + (ch['毛利率'] >= 25 ? '#059669' : '#f97316') + '">' + ch['毛利率'] + '%</td></tr>'
    ).join('')
    ...
    setTimeout(() => { win.print(); win.close() }, 300)
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex gap-1 bg-gray-100 rounded-lg p-1">
            {[{k:'month',l:'月'},{k:'quarter',l:'季'},{k:'year',l:'年'}].map(({k,l}) => (
              <button key={k} onClick={() => setRangeType(k)}
                className={`px-3 py-1.5 rounded-md text-sm font-medium transition-colors ${rangeType === k ? 'bg-white text-orange-500 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>{l}</button>
            ))}
          </div>
          ...
          <button onClick={handlePerformancePrint}
 ...
 + item.cost + '</td>' +
      '<td style="padding:7px 10px;text-align:right;font-weight:600;color:' + (item.marginRate >= 30 ? '#059669' : item.marginRate >= 20 ? '#f97316' : '#dc2626') + '">' + (item.marginRate !== null ? item.marginRate + '%' : '—') + '</td>' +
      '<td style="padding:7px 10px;text-align:right;font-weight:700;color:#059669">' + (item.totalProfit !== null ? fmt(item.totalProfit) : '—') + '</td></tr>'
    ).join('')
    ...
    const html = [
      ...
      '<div class="sec">',
      '<div class="sec-title">📊 通路毛利對比</div>',
      '<table><thead><tr><th>通路</th><th>營收</th><th>毛利</th><th>毛利率</th></tr></thead>',
      ...
      allItemStats.length > 0 ? [
        '<div class="sec">',
        '<div class="sec-title">🏆 品項全通路銷售總覽（Top 20）</div>',
        '<table><thead><tr><th>#</th><th>品項</th><th>電商</th><th>市集</th><th>實體</th><th>總計</th><th>銷售額</th></tr></thead>',
      ...
      itemMarginStats.length > 0 ? [
        '<div class="sec">',
        '<div class="sec-title">💰 品項毛利分析（Top 20）</div>',
        '<table><thead><tr><th>#</th><th>品項</th><th>售價</th><th>成本</th><th>毛利率</th><th>總獲利</th></tr></thead>',
